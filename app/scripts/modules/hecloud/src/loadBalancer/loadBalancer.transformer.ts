...
import {
  ...
  IHeCloudApplicationLoadBalancer,
  IHeCloudApplicationLoadBalancerUpsertCommand,
  IHeCloudClassicLoadBalancer,
  IHeCloudClassicLoadBalancerUpsertCommand,
  IHeCloudLoadBalancer,
  IHeCloudServerGroup,
  ...
  IHeCloudNetworkLoadBalancerUpsertCommand,
  ITargetGroup,
} from 'hecloud/domain';
import { VpcReader } from 'hecloud/vpc/VpcReader';
import { IPromise, module } from 'angular';
import { chain, filter, flatten, map } from 'lodash';

import { $q } from 'ngimport';

export class HeLoadBalancerTransformer {
  private updateHealthCounts(container: IServerGroup | ITargetGroup | IHeCloudLoadBalancer): void {
    ...
    if ((container as ITargetGroup | IHeCloudLoadBalancer).serverGroups) {
  ...
  private addVpcNameToContainer(
    container: IHeCloudLoadBalancer | ITargetGroup,
  ): (vpcs: IVpc[]) => IHeCloudLoadBalancer | ITargetGroup {
    return (vpcs: IVpc[]) => {
  ...
  public normalizeLoadBalancer(loadBalancer: IHeCloudLoadBalancer): IPromise<IHeCloudLoadBalancer> {
    ...
    if ((loadBalancer as IHeCloudApplicationLoadBalancer).targetGroups) {
      const appLoadBalancer = loadBalancer as IHeCloudApplicationLoadBalancer;
      appLoadBalancer.targetGroups.forEach(targetGroup => this.normalizeTargetGroup(targetGroup));
      serverGroups = flatten<IHeCloudServerGroup>(map(appLoadBalancer.targetGroups, 'serverGroups'));
    }
    ...
    return VpcReader.listVpcs().then(
      (vpcs: IVpc[]) => this.addVpcNameToContainer(loadBalancer)(vpcs) as IHeCloudLoadBalancer,
    );
  }
  ...
  public convertClassicLoadBalancerForEditing(
    ...
    const toEdit: IHeCloudClassicLoadBalancerUpsertCommand = {
  ...
  public convertApplicationLoadBalancerForEditing(
    ...
    const toEdit: IHeCloudApplicationLoadBalancerUpsertCommand = {
      availabilityZones: undefined,
  ...
  public convertNetworkLoadBalancerForEditing(
    ...
    // Since we build up toEdit as we go, much easier to declare as any, then cast at return time.
    const toEdit: IHeCloudNetworkLoadBalancerUpsertCommand = {
      availabilityZones: undefined,
      isInternal: loadBalancer.isInternal,
      region: loadBalancer.region,
  ...
  public constructNewClassicLoadBalancerTemplate(application: Application): IHeCloudClassicLoadBalancerUpsertCommand {
    const defaultCredentials = application.defaultCredentials.hecloud || HeProviderSettings.defaults.account,
      defaultRegion = application.defaultRegions.hecloud || HeProviderSettings.defaults.region,
      defaultSubnetType = HeProviderSettings.defaults.subnetType;
    return {
  ...
  public constructNewNetworkLoadBalancerTemplate(application: Application): IHeCloudNetworkLoadBalancerUpsertCommand {
    const defaultCredentials = application.defaultCredentials.hecloud || HeProviderSettings.defaults.account,
      defaultRegion = application.defaultRegions.hecloud || HeProviderSettings.defaults.region,
      defaultSubnetType = HeProviderSettings.defaults.subnetType,
      defaultTargetGroupName = `targetgroup`;
...
export const AWS_LOAD_BALANCER_TRANSFORMER = 'spinnaker.hecloud.loadBalancer.transformer';
module(AWS_LOAD_BALANCER_TRANSFORMER, []).service('hecloudLoadBalancerTransformer', HeLoadBalancerTransformer);
