...
import { IHeCloudLoadBalancer, IHeCloudLoadBalancerSourceData, IListenerAction, ITargetGroup } from 'hecloud/domain';
...
export class HeLoadBalancerDetailsController implements IController {
  ...
  public loadBalancer: IHeCloudLoadBalancer;
  ...
  public extractLoadBalancer(): IPromise<void> {
    ...
    if (appLoadBalancer) {
      ...
      return detailsLoader.then(
        (details: IHeCloudLoadBalancerSourceData[]) => {
...
export const AWS_LOAD_BALANCER_DETAILS_CTRL = 'spinnaker.hecloud.loadBalancer.details.controller';
module(AWS_LOAD_BALANCER_DETAILS_CTRL, [
  ...
  LOAD_BALANCER_READ_SERVICE,
  CONFIRMATION_MODAL_SERVICE,
]).controller('hecloudLoadBalancerDetailsCtrl', HeLoadBalancerDetailsController);
