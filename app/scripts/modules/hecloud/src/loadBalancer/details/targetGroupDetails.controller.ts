...
export class HeTargetGroupDetailsController implements IController {
  ...
  public loadBalancer: IHeCloudApplicationLoadBalancer;
  ...
  public extractTargetGroup(): IPromise<void> {
    ...
    const appLoadBalancer: IHeCloudApplicationLoadBalancer = this.app.loadBalancers.data.find((test: ILoadBalancer) => {
...
export const AWS_TARGET_GROUP_DETAILS_CTRL = 'spinnaker.hecloud.loadBalancer.details.targetGroupDetails.controller';
module(AWS_TARGET_GROUP_DETAILS_CTRL, [require('@uirouter/angularjs').default]).controller(
  'hecloudTargetGroupDetailsCtrl',
  HeTargetGroupDetailsController,
);
