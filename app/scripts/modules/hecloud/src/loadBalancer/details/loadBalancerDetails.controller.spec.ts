...
import { AWS_LOAD_BALANCER_DETAILS_CTRL, HeLoadBalancerDetailsController } from './loadBalancerDetails.controller';

describe('Controller: LoadBalancerDetailsCtrl', function() {
  let controller: HeLoadBalancerDetailsController;
  let $scope;
  let $state;
  const loadBalancer = {
    name: 'foo',
  ...
  beforeEach(
    mock.inject(($controller: IControllerService, $rootScope: IRootScopeService, _$state_: StateService) => {
      ...
      controller = $controller(HeLoadBalancerDetailsController, {
        $scope,
        loadBalancer,
        app,
        $state,
