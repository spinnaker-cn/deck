...
import { IHeCloudNetworkLoadBalancer, IHeCloudNetworkLoadBalancerUpsertCommand } from 'hecloud/domain';
...
export interface ICreateNetworkLoadBalancerProps extends ILoadBalancerModalProps {
  loadBalancer: IHeCloudNetworkLoadBalancer;
}

export interface ICreateApplicationLoadBalancerState {
  isNew: boolean;
  loadBalancerCommand: IHeCloudNetworkLoadBalancerUpsertCommand;
...
> {
  ...
  public static show(props: ICreateNetworkLoadBalancerProps): Promise<IHeCloudNetworkLoadBalancerUpsertCommand> {
  ...
  constructor(props: ICreateNetworkLoadBalancerProps) {
    ...
    const loadBalancerCommand = props.loadBalancer
      ? HeReactInjector.hecloudLoadBalancerTransformer.convertNetworkLoadBalancerForEditing(props.loadBalancer)
      : HeReactInjector.hecloudLoadBalancerTransformer.constructNewNetworkLoadBalancerTemplate(props.app);
  ...
  private formatListeners(command: IHeCloudNetworkLoadBalancerUpsertCommand): IPromise<void> {
  ...
  private setAvailabilityZones(loadBalancerCommand: IHeCloudNetworkLoadBalancerUpsertCommand): void {
  ...
  private manageTargetGroupNames(command: IHeCloudNetworkLoadBalancerUpsertCommand): void {
  ...
  private manageRules(command: IHeCloudNetworkLoadBalancerUpsertCommand): void {
    command.listeners.forEach(listener => {
  ...
  private formatCommand(command: IHeCloudNetworkLoadBalancerUpsertCommand): void {
    this.setAvailabilityZones(command);
    this.manageTargetGroupNames(command);
    this.manageRules(command);
  }

  protected onApplicationRefresh(values: IHeCloudNetworkLoadBalancerUpsertCommand): void {
  ...
  private onTaskComplete(values: IHeCloudNetworkLoadBalancerUpsertCommand): void {
  ...
  private submit = (values: IHeCloudNetworkLoadBalancerUpsertCommand): void => {
  ...
  private validate = (): FormikErrors<IHeCloudNetworkLoadBalancerUpsertCommand> => {
    const errors = {} as FormikErrors<IHeCloudNetworkLoadBalancerUpsertCommand>;
    return errors;
  };
  ...
  public render(): React.ReactElement<ICreateNetworkLoadBalancerProps> {
    ...
    return (
      <WizardModal<IHeCloudNetworkLoadBalancerUpsertCommand>
        heading={heading}
