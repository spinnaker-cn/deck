...
import { IHeCloudLoadBalancer, IHeCloudLoadBalancerUpsertCommand } from 'hecloud/domain';
import { SubnetSelectField } from 'hecloud/subnet';
...
export interface ILoadBalancerLocationProps {
  app: Application;
  formik: FormikProps<IHeCloudLoadBalancerUpsertCommand>;
  forPipelineConfig?: boolean;
  isNew?: boolean;
  loadBalancer?: IHeCloudLoadBalancer;
}
...
export class LoadBalancerLocation extends React.Component<ILoadBalancerLocationProps, ILoadBalancerLocationState>
  implements IWizardPageComponent<IHeCloudLoadBalancerUpsertCommand> {
  ...
  public validate(values: IHeCloudLoadBalancerUpsertCommand) {
    const errors = {} as FormikErrors<IHeCloudLoadBalancerUpsertCommand>;
  ...
  private shouldHideInternalFlag(): boolean {
    if (HeProviderSettings) {
      if (HeProviderSettings.loadBalancers && HeProviderSettings.loadBalancers.inferInternalFlagFromSubnet) {
  ...
  public componentDidMount(): void {
    ...
    const allAccounts$ = Observable.fromPromise(AccountService.listAccounts('hecloud')).shareReplay(1);
    ...
    const allLoadBalancers$ = this.props.app.getDataSource('loadBalancers').data$ as Observable<IHeCloudLoadBalancer[]>;
  ...
  private getAvailableSubnets(credentials: string, region: string): IPromise<ISubnet[]> {
    return SubnetReader.listSubnetsByProvider('hecloud').then(subnets => {
          ...
          <div className="modal-body">
            <div className="form-group">
              <div className={className}>
                ...
                <HelpField id="hecloud.loadBalancer.name" />
            ...
            <div className="form-group">
              <div className="col-md-3 sm-label-right">
                Stack <HelpField id="hecloud.loadBalancer.stack" />
              </div>
              ...
              <div className="col-md-6 form-inline">
                <label className="sm-label-right">
                  <span>
                    Detail <HelpField id="hecloud.loadBalancer.detail" />{' '}
                  </span>
                </label>
                <input
                  type="text"
            ...
            <SubnetSelectField
              labelColumns={3}
              helpKey="hecloud.loadBalancer.subnet"
              component={values}
              field="subnetId"
