...
import { IHeCloudApplicationLoadBalancer, IHeCloudHealth, IHeCloudServerGroup, ITargetGroup } from 'hecloud/domain';

export class HeCloudLoadBalancerDataUtils {
  private static buildTargetGroup(match: ITargetGroup, serverGroup: IHeCloudServerGroup): ITargetGroup {
    if (!match) {
      return null;
    }
    ...
    serverGroup.instances.forEach(instance => {
      const tgHealth: IHeCloudHealth = instance.health.find(h => h.type === 'TargetGroup') as IHeCloudHealth;
      if (tgHealth) {
  ...
  public static populateTargetGroups(
    ...
    return $q
      ...
      .then(data => {
        const hecloudAccount = (data[0] && data[0].hecloudAccount) || serverGroup.account;
        const loadBalancers: IHeCloudApplicationLoadBalancer[] = application
          .getDataSource('loadBalancers')
          .data.filter(
            lb => lb.loadBalancerType === 'application' || lb.loadBalancerType === 'network',
          ) as IHeCloudApplicationLoadBalancer[];
          ...
          ? serverGroup.targetGroups
              .map((targetGroupName: string) => {
                ...
                const targetGroup = allTargetGroups.find(
                  tg =>
                    tg.name === targetGroupName && tg.region === serverGroup.region && tg.account === hecloudAccount,
                );
