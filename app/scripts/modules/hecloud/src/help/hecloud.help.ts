...
const helpContents: { [key: string]: string } = {
  'hecloud.associateElasticIp.elasticIp': `<p>(Optional) <b>Elastic IP</b> is an IP address that Spinnaker will associate with this cluster.</p>
  ...
  'hecloud.associateElasticIp.type': `<p><b>Type</b> of elastic IP to associate:'
      <ul>
        <li><b>standard</b> is usable in EC2 Classic</li>
        <li><b>vpc</b> is usable in VPC</li>
      </ul>`,
  'hecloud.serverGroup.subnet': 'The subnet selection determines the subnet in which your instance will run.',
  'hecloud.loadBalancer.subnet': `The subnet selection determines the VPC in which your load balancer will run.<br/>
  ...
  'hecloud.loadBalancer.detail': `<p>(Optional) <b>Detail</b> is a string of free-form alphanumeric characters; by convention, we recommend using "frontend".</p>
  ...
  'hecloud.loadBalancer.name':
    '<p>The load balancer name is formed by combining the application name, the <b>Stack</b> field, and the <b>Detail</b> field.</p>',
  'hecloud.loadBalancer.targetGroups':
    'Add a target group if you want to associate this with an Application Load Balancer (ALB)',
  'hecloud.loadBalancer.loadBalancers':
    'And a load balancer directly if you created a Classic Load Balancer (a classic load balancer does not have target groups)',
  'hecloud.loadBalancer.ruleCondition.host': `<p>You can specify a single host name (for example, <em>my.example.com</em>). A host name is case insensitive, can be up to 128 characters in length, and can contain any of the following characters. Note that you can include up to three wildcard characters.</p>
      <ul>
        <li>A-Z, a-z, 0-9</li>
        <li>- .</li>
  ...
  'hecloud.loadBalancer.ruleCondition.path': `<p>You can specify a single path pattern (for example, <em>/img/*</em>). A path pattern is case sensitive, can be up to 128 characters in length, and can contain any of the following characters. Note that you can include up to three wildcard characters.</p>
      <ul>
        <li>A-Z, a-z, 0-9</li>
        <li>_ - . $ / ~ " ' @ : +</li>
        <li>& (using &amp;amp;)</li>
  ...
  'hecloud.loadBalancer.oidcAuthentication': 'Authentication requires a configured OIDC client.',
  'hecloud.serverGroup.stack':
    '(Optional) <b>Stack</b> is one of the core naming components of a cluster, used to create vertical stacks of dependent services for integration testing.',
  'hecloud.serverGroup.detail':
    '(Optional) <b>Detail</b> is a string of free-form alphanumeric characters and hyphens to describe any other variables.',
  'hecloud.serverGroup.imageName':
    '(Required) <b>Image</b> is the deployable HeCloud Machine Image. Images are restricted to the account and region selected.',
  'hecloud.serverGroup.legacyUdf': `<p>(Optional) <b>User Data Format</b> allows overriding of the format used when generating user data during deployment. The default format used is configured
  ...
  'hecloud.serverGroup.base64UserData': '(Optional) <b>UserData</b> is a base64 encoded string.',
  'hecloud.serverGroup.agency': '(Optional) <b>Agency</b> is the name of an agency.',
  'hecloud.serverGroup.enabledMetrics':
  ...
  'hecloud.serverGroup.instanceMonitoring':
  ...
  'hecloud.serverGroup.securityService':
    '(Optional) <b>Security Service</b> whether to enable security service of instances. Install the component to activate Anti-DDOS Protection and Instance Security (YunJing) at no cost.',
  'hecloud.serverGroup.tags': '(Optional) <b>Tags</b> are propagated to the instances in this cluster.',
  'hecloud.serverGroup.allImages': 'Search for an image that does not match the name of your application.',
  'hecloud.serverGroup.filterImages':
    'Select from a pre-filtered list of images matching the name of your application.',
  'hecloud.serverGroup.spotPrice': 'The maximum price to pay per hour per instance',
  'hecloud.serverGroup.traffic': `<p>Enables the "AddToLoadBalancer" scaling process, which is used by Spinnaker and discovery services to determine if the server group is enabled.</p>
     <p>Will be automatically enabled when any non "custom" deployment strategy is selected.</p>`,
  'hecloud.securityGroup.vpc': `
  ...
  'hecloud.blockDeviceMappings.useSource': `
    ...
    <p>In the event that there is no existing server group, the
        <a target="_blank" href="https://github.com/spinnaker/clouddriver/blob/master/clouddriver-hecloud/src/main/groovy/com/netflix/spinnaker/clouddriver/hecloud/deploy/BlockDeviceConfig.groovy">defaults</a>
        for the selected instance type will be used.</p>`,
  'hecloud.blockDeviceMappings.useAMI':
    '<p>Spinnaker will use the block device mappings from the selected AMI when deploying a new server group.</p>',
  'hecloud.blockDeviceMappings.useDefaults':
    '<p>Spinnaker will use the <a target="_blank" href="https://github.com/spinnaker/clouddriver/blob/master/clouddriver-hecloud/src/main/groovy/com/netflix/spinnaker/clouddriver/hecloud/deploy/BlockDeviceConfig.groovy">default block device mappings</a> for the selected instance type when deploying a new server group.</p>',
  'hecloud.targetGroup.protocol':
  ...
  'hecloud.targetGroup.targetType':
  ...
  'hecloud.targetGroup.attributes.deregistrationDelay':
  ...
  'hecloud.targetGroup.attributes.stickinessEnabled': ' Indicates whether sticky sessions are enabled.',
  'hecloud.targetGroup.attributes.stickinessType':
    'The type of sticky sessions. The only current possible value is <code>lb_cookie</code>.',
  'hecloud.targetGroup.attributes.stickinessDuration':
    'The time period, in seconds, during which requests from a client should be routed to the same target. After this time period expires, the load balancer-generated cookie is considered stale. The range is 1 second to 1 week (604800 seconds). The default value is 1 day (86400 seconds).',
  'hecloud.targetGroup.attributes.healthCheckPort.trafficPort':
  ...
  'hecloud.serverGroup.capacityConstraint': `
  ...
  'hecloud.tagImage.consideredStages': `Limit which previous stages will be considered when locating AMI's to tag. If left unchecked, AMI's generated by any upstream stage will be tagged.`,
  'hecloud.loadBalancer.redirect.host': `The hostname. This component is not percent-encoded. The hostname can contain <code>#{host}</code>.`,
  'hecloud.loadBalancer.redirect.path': `The absolute path, starting with the leading "/". This component is not percent-encoded. The path can contain <code>#{host}</code>, <code>#{path}</code>, and <code>#{port}</code>.`,
  'hecloud.loadBalancer.redirect.port': `The port. You can specify a value from 1 to 65535 or <code>#{port}</code>.`,
  'hecloud.loadBalancer.redirect.protocol': `The protocol. You can specify HTTP, HTTPS, or <code>#{protocol}</code>. You can redirect HTTP to HTTP, HTTP to HTTPS, and HTTPS to HTTPS. You cannot redirect HTTPS to HTTP.`,
  'hecloud.loadBalancer.redirect.query': `The query parameters, URL-encoded when necessary, but not percent-encoded. Do not include the leading "?", as it is automatically added. You can specify any of the reserved keywords.`,
  'hecloud.loadBalancer.redirect.statusCode': `The HTTP redirect code. The redirect is either permanent (HTTP 301) or temporary (HTTP 302).`,
  'hecloud.loadBalancer.redirect': `<p>A URI consists of the following components: protocol://hostname:port/path?query. You must modify at least one of the following components to avoid a redirect loop: protocol, hostname, port, or path. Any components that you do not modify retain their original values.
  ...
  `,
  'hecloud.cloudformation.expectedArtifact': `The artifact that is to be applied to this stage. The artifact should represent a valid cloudformation template.`,
};
