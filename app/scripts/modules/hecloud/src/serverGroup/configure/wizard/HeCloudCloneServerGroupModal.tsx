...
import { HeReactInjector } from '../../../reactShims';
...
export interface IHeCloudCloneServerGroupModalProps extends IModalComponentProps {
  title: string;
  application: Application;
  command: IHeCloudServerGroupCommand;
}

export interface IHeCloudCloneServerGroupModalState {
...
export class HeCloudCloneServerGroupModal extends React.Component<
  IHeCloudCloneServerGroupModalProps,
  IHeCloudCloneServerGroupModalState
> {
  public static defaultProps: Partial<IHeCloudCloneServerGroupModalProps> = {
    closeModal: noop,
    dismissModal: noop,
  };
  ...
  public static show(props: IHeCloudCloneServerGroupModalProps): Promise<IHeCloudServerGroupCommand> {
    const modalProps = { dialogClassName: 'wizard-modal modal-lg' };
    return ReactModal.show(HeCloudCloneServerGroupModal, props, modalProps);
  }

  constructor(props: IHeCloudCloneServerGroupModalProps) {
    super(props);
  ...
  private initializeCommand = () => {
    ...
    HeReactInjector.hecloudServerGroupConfigurationService.configureSubnetPurposes(command);
  };

  private configureCommand = () => {
    const { application, command } = this.props;
    HeReactInjector.hecloudServerGroupConfigurationService.configureCommand(application, command).then(() => {
      this.initializeCommand();
      this.setState({ loaded: true, requiresTemplateSelection: false });
    });
  };

  private normalizeCommand = ({ tags }: IHeCloudServerGroupCommand) => {
    if (!tags) {
      return;
    }
  ...
  private submit = (command: IHeCloudServerGroupCommand): void => {
  ...
  public render() {
    ...
    return (
      <WizardModal<IHeCloudServerGroupCommand>
        heading={title}
