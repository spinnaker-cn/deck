...
import { HeImageReader, IHeCloudImage } from '../../image';

export interface IHeCloudImageSelectorProps {
  onChange: (value: IHeCloudImage) => void;
  value: IHeCloudImage;
...
export interface IHeCloudImageSelectorState {
  errorMessage?: string;
  selectionMode: 'packageImages' | 'searchAllImages';
  searchString: string;
  searchResults: IHeCloudImage[];
  isSearching: boolean;
  packageImages: IHeCloudImage[];
  isLoadingPackageImages: boolean;
}

type sortImagesByOptions = 'name' | 'ts';

export class HeCloudImageSelectInput extends React.Component<IHeCloudImageSelectorProps, IHeCloudImageSelectorState> {
  public state: IHeCloudImageSelectorState = {
    errorMessage: null,
  ...
  private hecloudImageReader = new HeImageReader();
  private props$ = new Subject<IHeCloudImageSelectorProps>();
  private searchInput$ = new Subject<string>();
  private destroy$ = new Subject();
  private sortImagesBy$ = new BehaviorSubject<sortImagesByOptions>('ts');

  public static makeFakeImage(imageName: string, imageId: string, region: string): IHeCloudImage {
    ...
    return { imageName, imgIds, attributes } as IHeCloudImage;
  }

  private loadImagesFromApplicationName(application: Application): IPromise<IHeCloudImage[]> {
    const query = application.name.replace(/_/g, '[_\\-]') + '*';
    return this.hecloudImageReader.findImages({ q: query });
  }
  ...
  private loadImageById(imageId: string, region: string, credentials: string): IPromise<IHeCloudImage> {
    return !imageId ? $q.when(null) : this.hecloudImageReader.getImage(imageId, region, credentials).catch(() => null);
  }

  private searchForImages(query: string): IPromise<IHeCloudImage[]> {
    const hasMinLength = query && query.length >= 3;
    return hasMinLength ? this.hecloudImageReader.findImages({ q: query }) : $q.when([]);
  }
  ...
  private selectImage(selectedImage: IHeCloudImage) {
    if (this.props.value !== selectedImage) {
      this.props.onChange(selectedImage);
    }
  }

  private findMatchingImage(images: IHeCloudImage[], selectedImage: IHeCloudImage) {
  ...
  public componentDidMount() {
    ...
    const packageImages$ = Observable.fromPromise(fetchPromise)
      .catch(err => {
        ...
        return Observable.of([] as IHeCloudImage[]);
      })
    ...
    const searchImagesInRegion$ = searchImages$
      ...
      .map(([searchResults, latestRegion, sortImagesBy]) => {
        ...
        if (searchResults.length === 0 && !!/img-[0-9a-f]{8}/.exec(searchString)) {
          const fakeImage = HeCloudImageSelectInput.makeFakeImage(searchString, searchString, latestRegion);
  ...
  private sortImages(images: IHeCloudImage[], sortImagesBy: sortImagesByOptions): IHeCloudImage[] {
  ...
  public render() {
    ...
    const ImageOptionRenderer = (image: IHeCloudImage) => {
    ...
    if (selectionMode === 'searchAllImages') {
      ...
      return (
        <div className="col-md-7">
          ...
          <HelpField id="hecloud.serverGroup.allImages" />
        </div>
      );
    } else {
