...
import { HeModalFooter } from 'hecloud/common';
import { IHeCloudAsg, IHeCloudServerGroup } from 'hecloud/domain';

export interface IHeCloudResizeServerGroupModalProps extends IModalComponentProps {
  application: Application;
  serverGroup: IHeCloudServerGroup;
}

export interface IHeCloudResizeServerGroupModalState {
  advancedMode: boolean;
  interestingHealthProviderNames: string[]; // managed by a separate component
  initialValues: IHeCloudResizeServerGroupValues;
  taskMonitor: TaskMonitor;
  platformHealthOnlyShowOverride?: boolean;
}

export interface IHeCloudResizeServerGroupValues {
  min: number;
  max: number;
  desired: number;
...
export class HeCloudResizeServerGroupModal extends React.Component<
  IHeCloudResizeServerGroupModalProps,
  IHeCloudResizeServerGroupModalState
> {
  public static defaultProps: Partial<IHeCloudResizeServerGroupModalProps> = {
    closeModal: noop,
    dismissModal: noop,
  };

  private formikRef = React.createRef<Formik<IHeCloudResizeServerGroupValues>>();

  public static show(props: IHeCloudResizeServerGroupModalProps): Promise<IResizeJob> {
    const modalProps = {};
    return ReactModal.show(HeCloudResizeServerGroupModal, props, modalProps);
  }

  constructor(props: IHeCloudResizeServerGroupModalProps) {
    super(props);
    ...
    this.state = {
      ...
      interestingHealthProviderNames:
        attributes.platformHealthOnlyShowOverride && attributes.platformHealthOnly ? ['HeCloud'] : null,
    };
  }

  private getChangedFields(capacity: ICapacity, asg: IHeCloudAsg): IChangedField[] {
  ...
  private validate = (
    values: IHeCloudResizeServerGroupValues,
  ): Partial<FormikErrors<IHeCloudResizeServerGroupValues>> => {
    const { min, max, desired } = values;
    const { asg } = this.props.serverGroup;
    const errors: Partial<FormikErrors<IHeCloudResizeServerGroupValues>> = {};
  ...
  private submit = (values: IHeCloudResizeServerGroupValues): void => {
  ...
  private renderSimpleMode(formik: FormikProps<IHeCloudResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
    const { asg } = serverGroup;
    return (
      <div>
  ...
  private renderAdvancedMode(formik: FormikProps<IHeCloudResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
    const { errors } = formik;
    const { asg } = serverGroup;
          ...
          <FormikFormField
            ...
            input={props => (
              <>
                ...
                <HelpField id="hecloud.serverGroup.capacityConstraint" />
              </>
            )}
          />
        </div>
  ...
  private renderScalingPolicyWarning(formik: FormikProps<IHeCloudResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
  ...
  public render() {
    ...
    return (
      <>
        ...
        <Formik<IHeCloudResizeServerGroupValues>
          ref={this.formikRef}
