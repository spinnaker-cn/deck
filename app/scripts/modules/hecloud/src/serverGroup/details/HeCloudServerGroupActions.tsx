...
import { IHeCloudServerGroup, IHeCloudServerGroupView } from 'hecloud/domain';
import { HeCloudCloneServerGroupModal } from 'hecloud/serverGroup/configure/wizard/HeCloudCloneServerGroupModal';
import { HeReactInjector } from 'hecloud/reactShims';
import { IHeCloudServerGroupCommand } from '../configure';
import { HeCloudResizeServerGroupModal } from './resize/HeCloudResizeServerGroupModal';

export interface IHeCloudServerGroupActionsProps extends IServerGroupActionsProps {
  serverGroup: IHeCloudServerGroupView;
}

export class HeCloudServerGroupActions extends React.Component<IHeCloudServerGroupActionsProps> {
  ...
  private destroyServerGroup = (): void => {
    ...
    if (app.attributes.platformHealthOnlyShowOverride && app.attributes.platformHealthOnly) {
      confirmationModalParams.interestingHealthProviderNames = ['HeCloud'];
    }
  ...
  private rollbackServerGroup = (): void => {
    ...
    let serverGroup: IHeCloudServerGroup = this.props.serverGroup;
    let previousServerGroup: IHeCloudServerGroup;
    ...
    if (serverGroup.isDisabled) {
       ...
       */
      serverGroup = orderBy(
        allServerGroups.filter((g: IHeCloudServerGroup) => g.name !== previousServerGroup.name && !g.isDisabled),
        ['instanceCounts.total', 'createdTime'],
        ['desc', 'desc'],
      )[0] as IHeCloudServerGroup;
    }

    // the set of all server groups should not include the server group selected for rollback
    allServerGroups = allServerGroups.filter((g: IHeCloudServerGroup) => g.name !== serverGroup.name);
    ...
    ModalInjector.modalService.open({
      templateUrl: ReactInjector.overrideRegistry.getTemplate(
        'hecloud.rollback.modal',
        require('./rollback/rollbackServerGroup.html'),
      ),
      controller: 'hecloudRollbackServerGroupCtrl as ctrl',
      resolve: {
  ...
  private resizeServerGroup = (): void => {
    const { app, serverGroup } = this.props;
    HeCloudResizeServerGroupModal.show({ application: app, serverGroup });
  };

  private cloneServerGroup = (): void => {
    const { app, serverGroup } = this.props;
    HeReactInjector.hecloudServerGroupCommandBuilder
      .buildServerGroupCommandFromExisting(app, serverGroup)
      .then((command: IHeCloudServerGroupCommand) => {
        const title = `Clone ${serverGroup.name}`;
        HeCloudCloneServerGroupModal.show({ title, application: app, command });
      });
  };
