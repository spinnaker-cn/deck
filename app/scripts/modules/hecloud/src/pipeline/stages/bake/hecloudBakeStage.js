...
module.exports = angular
  .module('spinnaker.hecloud.pipeline.stage.bakeStage', [require('./bakeExecutionDetails.controller').name])
  .config(function() {
  ...
  .controller('hecloudBakeStageCtrl', [
    '$scope',
    '$q',
    '$uibModal',
    function($scope, $q, $uibModal) {
      ...
      $scope.viewState = {
        ...
        minRootVolumeSize: HeProviderSettings.minRootVolumeSize,
      };

      function initialize() {
        $q.all({
          regions: BakeryReader.getRegions('hecloud'),
          baseOsOptions: BakeryReader.getBaseOsOptions('hecloud'),
          ...
          if (!$scope.stage.regions.length && $scope.application.defaultRegions.hecloud) {
            $scope.stage.regions.push($scope.application.defaultRegions.hecloud);
          }
          if (!$scope.stage.regions.length && $scope.application.defaultRegions.hecloud) {
            $scope.stage.regions.push($scope.application.defaultRegions.hecloud);
          }
