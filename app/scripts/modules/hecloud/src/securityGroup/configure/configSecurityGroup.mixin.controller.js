...
module.exports = angular
  .module('spinnaker.hecloud.securityGroup.baseConfig.controller', [
    require('@uirouter/angularjs').default,
    SECURITY_GROUP_READER,
  ])
  .controller('hecloudConfigSecurityGroupMixin', [
    '$scope',
    '$state',
    '$uibModalInstance',
    'application',
    ...
    function($scope, $state, $uibModalInstance, application, securityGroup, securityGroupReader) {
      ...
      ctrl.initializeAccounts = () => {
        return AccountService.listAllAccounts('hecloud').then(function(accounts) {
      ...
      function configureFilteredSecurityGroups() {
        ...
        var regionalGroupNames = _.get(allSecurityGroups, [account, 'hecloud', region].join('.'), []).map(
          sg => sg.name,
        );
      ...
      ctrl.initializeSecurityGroups = function() {
        return securityGroupReader.getAllSecurityGroups().then(function(securityGroups) {
          ...
          var availableGroups;
          if (account && region) {
            availableGroups = (securityGroups[account] && securityGroups[account].hecloud[region]) || [];
          } else {
