...
class IngressRuleSelectorController implements IController {
  ...
  public setAvailableSecurityGroups(): void {
    ...
    regions.forEach(region => {
      ...
      const regionalGroupNames = get(this.allSecurityGroups, [account, 'hecloud', region].join('.'), [])
...
export const INGRESS_RULE_GROUP_SELECTOR_COMPONENT =
  'spinnaker.hecloud.securityGroup.configure.ingressRuleGroupSelector';
module(INGRESS_RULE_GROUP_SELECTOR_COMPONENT, []).component('hecloudIngressRuleGroupSelector', ingressRuleSelector);
