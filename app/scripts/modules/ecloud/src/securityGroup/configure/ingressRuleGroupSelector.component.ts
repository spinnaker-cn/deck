...
class IngressRuleSelectorController implements IController {
  ...
  public setAvailableSecurityGroups(): void {
    ...
    regions.forEach(region => {
      ...
      const regionalGroupNames = get(this.allSecurityGroups, [account, 'ecloud', region].join('.'), [])
...
export const INGRESS_RULE_GROUP_SELECTOR_COMPONENT =
  'spinnaker.ecloud.securityGroup.configure.ingressRuleGroupSelector';
module(INGRESS_RULE_GROUP_SELECTOR_COMPONENT, []).component('ecloudIngressRuleGroupSelector', ingressRuleSelector);
