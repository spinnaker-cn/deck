...
import { VpcReader } from 'ecloud/vpc/VpcReader';
...
export class AwsLoadBalancerTransformer {
  ...
  public constructNewClassicLoadBalancerTemplate(application: Application): IAmazonClassicLoadBalancerUpsertCommand {
    const defaultCredentials = application.defaultCredentials.ecloud || AWSProviderSettings.defaults.account,
      defaultRegion = application.defaultRegions.ecloud || AWSProviderSettings.defaults.region,
...
export const AWS_LOAD_BALANCER_TRANSFORMER = 'spinnaker.ecloud.loadBalancer.transformer';
module(AWS_LOAD_BALANCER_TRANSFORMER, []).service('ecloudLoadBalancerTransformer', AwsLoadBalancerTransformer);
