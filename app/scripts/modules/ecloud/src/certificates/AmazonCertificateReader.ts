...
import { IAmazonCertificate } from 'ecloud/domain';

export class AmazonCertificateReader {
  public static listCertificates(): IPromise<{ [accountId: string]: IAmazonCertificate[] }> {
    return CertificateReader.listCertificatesByProvider('ecloud').then((certificates: IAmazonCertificate[]) => {
      // This account grouping should really go into clouddriver but since it's not, put it here for now.
      return AccountService.listAllAccounts('ecloud').then(allAccountDetails => {
