...
import { IScheduledAction } from 'ecloud/domain';
    ...
    ReactInjector.confirmationModalService.confirm({
      ...
      submitMethod: () => {
        return TaskExecutor.executeTask({
          job: [
            {
              type: 'deleteEcloudScheduledAction',
    ...
    return (
      <div>
        <dl className="horizontal-when-filters-collapsed" style={{ marginBottom: '20px' }}>
          ...
          <dd>{action.scheduledActionId}({action.scheduledActionName})</dd>

          <dt>Start Time</dt><dd>{action.startTime}</dd>
          <dt>End Time</dt><dd>{action.endTime}</dd>
          <dt>Real scaling Size</dt><dd>{action.realScalingSize}</dd>
        ...
        {/* <div className="actions text-right">
          <button className="btn btn-xs btn-link" onClick={() => {this.deleteScheduledAction(action)}} >
