...
> {
  ...
  private initializeCommand = () => {
    const { command } = this.props;
       if(command.keyPair && !command.keyPairName){
            command.keyPairName = command.keyPair;
            const keyPairsArr = (command.backingData.keyPairs || []).find(({ keyName }) => keyName === command.keyPair);
            command.keyPair = keyPairsArr.keyId;
       }
    command.credentialsChanged(command);
    command.regionChanged(command);
    AwsReactInjector.ecloudServerGroupConfigurationService.configureSubnetPurposes(command);
  };

  private configureCommand = () => {
    const { application, command } = this.props;
    AwsReactInjector.ecloudServerGroupConfigurationService.configureCommand(application, command).then(() => {
      this.initializeCommand();

      if(command.forwardLoadBalancers&& command.forwardLoadBalancers.length){
        command.forwardLoadBalancers.map(i =>{
          command.backingData.subnets.map(k =>{
            if(i.subnetId === k.id){
              i.zone = k.zone;
            }
          })
      ...
      if(command.subnetChanged){
        command.subnetChanged(command,command.subnetIds);
      }
