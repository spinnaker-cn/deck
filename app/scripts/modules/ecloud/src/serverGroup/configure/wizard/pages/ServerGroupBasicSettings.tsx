...
import { SubnetSelectField } from 'ecloud/subnet';
...
export class ServerGroupBasicSettings
  ...
  private imageChanged = (image: IAmazonImage) => {
    ...
    if (image) {
      ...
      setFieldValue('isPublic', image.attributes.isPublic || 0);
  ...
  private subnetUpdated = (): void => {
    ...
    values.subnetChanged(values,values.subnetIds);
  };
  ...
  public validate(values: IAmazonServerGroupCommand): { [key: string]: string } {
    ...
    if (!values.multiRegionCreatePolicy) {
      errors.multiRegionCreatePolicy = '多可用区扩容策略必填.';
    }
  ...
  private policyChange = (value: string) => {
    this.props.formik.setFieldValue('multiRegionCreatePolicy', value);
  };
        ...
        <div className="form-group">
          ...
          <div className="col-md-3 radio">
            <label>
              <input
                type="radio"
                checked={values.multiRegionCreatePolicy === 'PRIORITY'}
                onChange={() =>{
                  this.policyChange("PRIORITY");
                }}
              />
              优先级策略
            </label>
          ...
          <div className="col-md-3 radio">
            <label>
              <input
                type="radio"
                checked={values.multiRegionCreatePolicy === 'BALANCE'}
                onChange={() =>{
                  this.policyChange("BALANCE");
                }}
              />
              均衡分布策略
            </label>
