...
class AmazonApplicationNameValidator implements IApplicationNameValidator {
  ...
  private validateLength(name: string, warnings: string[], errors: string[]) {
    if (name.length > 250) {
      errors.push('The maximum length for an application in Ecloud is 250 characters.');
      return;
    }
    if (name.length > 240) {
      if (name.length >= 248) {
    ...
    if (name.length > 20) {
      if (name.length > 32) {
        warnings.push(`You will not be able to create an Ecloud load balancer for this application if the
...
ApplicationNameValidator.registerValidator('ecloud', new AmazonApplicationNameValidator());
