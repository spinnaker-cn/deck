...
module.exports = angular
  .module('spinnaker.huaweicloud.securityGroup.baseConfig.controller', [
    require('@uirouter/angularjs').default,
    SECURITY_GROUP_READER,
  ])
  .controller('huaweicloudConfigSecurityGroupMixin', [
    '$scope',
    '$state',
    '$uibModalInstance',
    'application',
    ...
    function($scope, $state, $uibModalInstance, application, securityGroup, securityGroupReader) {
      ...
      ctrl.initializeAccounts = () => {
        return AccountService.listAllAccounts('huaweicloud').then(function(accounts) {
      ...
      function configureFilteredSecurityGroups() {
        ...
        var regionalGroupNames = _.get(allSecurityGroups, [account, 'huaweicloud', region].join('.'), []).map(
          sg => sg.name,
        );
      ...
      ctrl.initializeSecurityGroups = function() {
        return securityGroupReader.getAllSecurityGroups().then(function(securityGroups) {
          ...
          var availableGroups;
          if (account && region) {
            availableGroups = (securityGroups[account] && securityGroups[account].huaweicloud[region]) || [];
          } else {
