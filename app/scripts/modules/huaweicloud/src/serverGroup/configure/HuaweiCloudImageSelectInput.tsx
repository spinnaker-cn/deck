...
import { HuaweiImageReader, IHuaweiCloudImage } from '../../image';

export interface IHuaweiCloudImageSelectorProps {
  onChange: (value: IHuaweiCloudImage) => void;
  value: IHuaweiCloudImage;
  application: Application;
  credentials: string;
  region: string;
}

export interface IHuaweiCloudImageSelectorState {
  errorMessage?: string;
  selectionMode: 'packageImages' | 'searchAllImages';
  searchString: string;
  searchResults: IHuaweiCloudImage[];
  isSearching: boolean;
  packageImages: IHuaweiCloudImage[];
  isLoadingPackageImages: boolean;
}

type sortImagesByOptions = 'name' | 'ts';

export class HuaweiCloudImageSelectInput extends React.Component<IHuaweiCloudImageSelectorProps, IHuaweiCloudImageSelectorState> {
  public state: IHuaweiCloudImageSelectorState = {
    errorMessage: null,
  ...
  private huaweicloudImageReader = new HuaweiImageReader();
  private props$ = new Subject<IHuaweiCloudImageSelectorProps>();
  private searchInput$ = new Subject<string>();
  private destroy$ = new Subject();
  private sortImagesBy$ = new BehaviorSubject<sortImagesByOptions>('ts');

  public static makeFakeImage(imageName: string, imageId: string, region: string): IHuaweiCloudImage {
    ...
    return { imageName, imgIds, attributes } as IHuaweiCloudImage;
  }

  private loadImagesFromApplicationName(application: Application): IPromise<IHuaweiCloudImage[]> {
    const query = application.name.replace(/_/g, '[_\\-]') + '*';
    return this.huaweicloudImageReader.findImages({ q: query });
  }
  ...
  private loadImageById(imageId: string, region: string, credentials: string): IPromise<IHuaweiCloudImage> {
    return !imageId ? $q.when(null) : this.huaweicloudImageReader.getImage(imageId, region, credentials).catch(() => null);
  }

  private searchForImages(query: string): IPromise<IHuaweiCloudImage[]> {
    const hasMinLength = query && query.length >= 3;
    return hasMinLength ? this.huaweicloudImageReader.findImages({ q: query }) : $q.when([]);
  }
  ...
  private selectImage(selectedImage: IHuaweiCloudImage) {
    if (this.props.value !== selectedImage) {
      this.props.onChange(selectedImage);
    }
  }

  private findMatchingImage(images: IHuaweiCloudImage[], selectedImage: IHuaweiCloudImage) {
  ...
  public componentDidMount() {
    ...
    const packageImages$ = Observable.fromPromise(fetchPromise)
      .catch(err => {
        ...
        return Observable.of([] as IHuaweiCloudImage[]);
      })
    ...
    const searchImagesInRegion$ = searchImages$
      ...
      .map(([searchResults, latestRegion, sortImagesBy]) => {
        ...
        if (searchResults.length === 0 && !!/img-[0-9a-f]{8}/.exec(searchString)) {
          const fakeImage = HuaweiCloudImageSelectInput.makeFakeImage(searchString, searchString, latestRegion);
  ...
  private sortImages(images: IHuaweiCloudImage[], sortImagesBy: sortImagesByOptions): IHuaweiCloudImage[] {
  ...
  public render() {
    ...
    const ImageOptionRenderer = (image: IHuaweiCloudImage) => {
    ...
    if (selectionMode === 'searchAllImages') {
      ...
      return (
        <div className="col-md-7">
          ...
          <HelpField id="huaweicloud.serverGroup.allImages" />
        </div>
      );
    } else {
