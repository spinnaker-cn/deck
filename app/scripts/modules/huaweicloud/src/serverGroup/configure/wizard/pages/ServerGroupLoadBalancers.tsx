...
import {
  IHuaweiCloudServerGroupCommand,
  IHuaweiCloudForwardLoadBalancer,
  IHuaweiCloudLbPoolMap,
} from '../../serverGroupConfiguration.service';

import { IALBPool } from 'huaweicloud/domain';

export interface IServerGroupLoadBalancersProps {
  formik: FormikProps<IHuaweiCloudServerGroupCommand>;
}
interface IHuaweiCloudLocation {
  selectedPool: IALBPool;
}
interface IHuaweiCloudLocationMap {
  [key: string]: IHuaweiCloudLocation;
}
export interface IServerGroupLoadBalancersState {
  refreshing: boolean;
  poolLocationMap: IHuaweiCloudLocationMap;
}
...
export class ServerGroupLoadBalancers
  ...
  public state = {
    refreshing: false,
    poolLocationMap: {} as IHuaweiCloudLocationMap,
  };
  ...
  private loadBalancerChanged(forwardLoadBalancer: IHuaweiCloudForwardLoadBalancer, loadBalancerId: string): void {
  ...
  private poolChanged(forwardLoadBalancer: IHuaweiCloudForwardLoadBalancer, poolId: string, index: number) {
  ...
  private portChanged(forwardLoadBalancer: IHuaweiCloudForwardLoadBalancer, port: number) {
    forwardLoadBalancer.targetAttributes[0].port = port;
    this.updateLoadBalancers();
  }

  private weightChanged(forwardLoadBalancer: IHuaweiCloudForwardLoadBalancer, weight: number) {
  ...
  public componentWillReceiveProps(nextProps: IServerGroupLoadBalancersProps): void {
    const {
      values: {
        ...
        backingData: {
          filtered: { lbPoolMap = {} as IHuaweiCloudLbPoolMap },
        },
    ...
    if (
      ...
      this.setState({
        poolLocationMap: Object.assign(
          {},
          this.state.poolLocationMap,
          forwardLoadBalancers.reduce((p: IHuaweiCloudLocationMap, c, index) => {
