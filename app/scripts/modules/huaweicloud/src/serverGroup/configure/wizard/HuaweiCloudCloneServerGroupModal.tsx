...
import { HuaweiReactInjector } from '../../../reactShims';
...
export interface IHuaweiCloudCloneServerGroupModalProps extends IModalComponentProps {
  title: string;
  application: Application;
  command: IHuaweiCloudServerGroupCommand;
}

export interface IHuaweiCloudCloneServerGroupModalState {
...
export class HuaweiCloudCloneServerGroupModal extends React.Component<
  IHuaweiCloudCloneServerGroupModalProps,
  IHuaweiCloudCloneServerGroupModalState
> {
  public static defaultProps: Partial<IHuaweiCloudCloneServerGroupModalProps> = {
    closeModal: noop,
    dismissModal: noop,
  };
  ...
  public static show(props: IHuaweiCloudCloneServerGroupModalProps): Promise<IHuaweiCloudServerGroupCommand> {
    const modalProps = { dialogClassName: 'wizard-modal modal-lg' };
    return ReactModal.show(HuaweiCloudCloneServerGroupModal, props, modalProps);
  }

  constructor(props: IHuaweiCloudCloneServerGroupModalProps) {
    super(props);
  ...
  private initializeCommand = () => {
    ...
    HuaweiReactInjector.huaweicloudServerGroupConfigurationService.configureSubnetPurposes(command);
  };

  private configureCommand = () => {
    const { application, command } = this.props;
    HuaweiReactInjector.huaweicloudServerGroupConfigurationService.configureCommand(application, command).then(() => {
      this.initializeCommand();
      this.setState({ loaded: true, requiresTemplateSelection: false });
    });
  };

  private normalizeCommand = ({ tags }: IHuaweiCloudServerGroupCommand) => {
    if (!tags) {
      return;
    }
  ...
  private submit = (command: IHuaweiCloudServerGroupCommand): void => {
  ...
  public render() {
    ...
    return (
      <WizardModal<IHuaweiCloudServerGroupCommand>
        heading={title}
