...
describe('AutoScalingProcessService', () => {
  describe('normalizeScalingProcesses', () => {
    it('returns an empty list if no asg or suspendedProcesses present on server group', function() {
      expect(AutoScalingProcessService.normalizeScalingProcesses({} as IHuaweiCloudServerGroup)).toEqual([]);
      expect(AutoScalingProcessService.normalizeScalingProcesses({ asg: {} } as IHuaweiCloudServerGroup)).toEqual([]);
    });
    ...
    it('returns all processes normalized if suspendedProcesses is empty', function() {
      ...
      const normalized = AutoScalingProcessService.normalizeScalingProcesses({ asg: asg } as IHuaweiCloudServerGroup);
  ...
  describe('getDisabledDate', function() {
    it('returns null when server group is not disabled, regardless of suspended processes', function() {
      ...
      expect(AutoScalingProcessService.getDisabledDate({ asg: asg } as IHuaweiCloudServerGroup)).toBeNull();
    });
    ...
    it('returns null when server group is disabled but suspended process for AddToLoadBalancer not present', function() {
      ...
      expect(
        AutoScalingProcessService.getDisabledDate({ isDisabled: true, asg: asg } as IHuaweiCloudServerGroup),
      ).toBeNull();
    });
    ...
    it('returns suspension date when server group is disabled and AddToLoadBalancer suspended', function() {
      ...
      expect(AutoScalingProcessService.getDisabledDate({ isDisabled: true, asg: asg } as IHuaweiCloudServerGroup)).toEqual(
        1452639586000,
      );
    });
  });
