...
import { HuaweiModalFooter } from 'huaweicloud/common';
import { IHuaweiCloudAsg, IHuaweiCloudServerGroup } from 'huaweicloud/domain';

export interface IHuaweiCloudResizeServerGroupModalProps extends IModalComponentProps {
  application: Application;
  serverGroup: IHuaweiCloudServerGroup;
}

export interface IHuaweiCloudResizeServerGroupModalState {
  advancedMode: boolean;
  interestingHealthProviderNames: string[]; // managed by a separate component
  initialValues: IHuaweiCloudResizeServerGroupValues;
  taskMonitor: TaskMonitor;
  platformHealthOnlyShowOverride?: boolean;
}

export interface IHuaweiCloudResizeServerGroupValues {
  min: number;
  max: number;
  desired: number;
...
export class HuaweiCloudResizeServerGroupModal extends React.Component<
  IHuaweiCloudResizeServerGroupModalProps,
  IHuaweiCloudResizeServerGroupModalState
> {
  public static defaultProps: Partial<IHuaweiCloudResizeServerGroupModalProps> = {
    closeModal: noop,
    dismissModal: noop,
  };

  private formikRef = React.createRef<Formik<IHuaweiCloudResizeServerGroupValues>>();

  public static show(props: IHuaweiCloudResizeServerGroupModalProps): Promise<IResizeJob> {
    const modalProps = {};
    return ReactModal.show(HuaweiCloudResizeServerGroupModal, props, modalProps);
  }

  constructor(props: IHuaweiCloudResizeServerGroupModalProps) {
    super(props);
    ...
    this.state = {
      ...
      interestingHealthProviderNames:
        attributes.platformHealthOnlyShowOverride && attributes.platformHealthOnly ? ['HuaweiCloud'] : null,
    };
  }

  private getChangedFields(capacity: ICapacity, asg: IHuaweiCloudAsg): IChangedField[] {
  ...
  private validate = (
    values: IHuaweiCloudResizeServerGroupValues,
  ): Partial<FormikErrors<IHuaweiCloudResizeServerGroupValues>> => {
    const { min, max, desired } = values;
    const { asg } = this.props.serverGroup;
    const errors: Partial<FormikErrors<IHuaweiCloudResizeServerGroupValues>> = {};
  ...
  private isDesiredControlledByAutoscaling = (): boolean => {
    ...
    return (
      scalingPolicies.length && advancedMode && scalingPolicies.every(p => p.adjustmentType !== 'EXACT_CAPACITY')
    );
  };

  private submit = (values: IHuaweiCloudResizeServerGroupValues): void => {
  ...
  private renderSimpleMode(formik: FormikProps<IHuaweiCloudResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
    const { asg } = serverGroup;
    return (
      <div>
  ...
  private renderAdvancedMode(formik: FormikProps<IHuaweiCloudResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
    const { errors } = formik;
    const { asg } = serverGroup;
          ...
          <FormikFormField
            ...
            input={props => (
              <>
                ...
                <HelpField id="huaweicloud.serverGroup.capacityConstraint" />
              </>
            )}
          />
        </div>
  ...
  private renderScalingPolicyWarning(formik: FormikProps<IHuaweiCloudResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
  ...
  public render() {
    ...
    return (
      <>
        ...
        <Formik<IHuaweiCloudResizeServerGroupValues>
          ref={this.formikRef}
