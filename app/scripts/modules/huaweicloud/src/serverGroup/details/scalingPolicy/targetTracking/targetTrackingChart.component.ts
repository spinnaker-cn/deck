...
import { IScalingPolicyAlarm, ITargetTrackingConfiguration } from 'huaweicloud/domain';
...
const component: IComponentOptions = {
  ...
  template: `
    <huaweicloud-metric-alarm-chart alarm="$ctrl.alarm"
                        style="height: 150px;"
                        alarm-updated="$ctrl.alarmUpdated"
                        ticks="{x: 12, y: 5}"
                        margins="{top: 10, left: 50}"
                        stats="$ctrl.state"
                        server-group="$ctrl.serverGroup"></huaweicloud-metric-alarm-chart>
  `,
};

export const TARGET_TRACKING_CHART_COMPONENT = 'spinnaker.huaweicloud.scalingPolicy.targetTracking.chart.component';
