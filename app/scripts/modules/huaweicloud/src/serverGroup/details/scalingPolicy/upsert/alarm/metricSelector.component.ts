...
import { IConfigurableMetric } from 'huaweicloud/serverGroup';
...
export class MetricSelectorController implements IController {
  ...
  public namespaces = get(HuaweiProviderSettings, 'metrics.customNamespaces', []).concat(NAMESPACES);
  ...
  public updateAvailableMetrics(): void {
    const { alarm } = this;
    Promise.resolve(this.metricNames.map(mn => ({
      ...
      <div style="display: inline-block; width: 500px" ng-if="$ctrl.state.metricsLoaded">
        ...
        <div style="padding-left: 5px;">
          <a href class="small"
            ...
            Only show metrics for this auto scaling group <help-field key="huaweicloud.scalingPolicy.search.restricted"></help-field>
          </a>
        </div>
      </div>`,
};

export const METRIC_SELECTOR_COMPONENT = 'spinnaker.huaweicloud.scalingPolicy.alarm.metric.editor';
module(METRIC_SELECTOR_COMPONENT, []).component('huaweicloudMetricSelector', component);
