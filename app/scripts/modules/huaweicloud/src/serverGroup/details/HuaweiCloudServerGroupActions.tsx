...
import { IHuaweiCloudServerGroup, IHuaweiCloudServerGroupView } from 'huaweicloud/domain';
import { HuaweiCloudCloneServerGroupModal } from 'huaweicloud/serverGroup/configure/wizard/HuaweiCloudCloneServerGroupModal';
import { HuaweiReactInjector } from 'huaweicloud/reactShims';
import { IHuaweiCloudServerGroupCommand } from '../configure';
import { HuaweiCloudResizeServerGroupModal } from './resize/HuaweiCloudResizeServerGroupModal';

export interface IHuaweiCloudServerGroupActionsProps extends IServerGroupActionsProps {
  serverGroup: IHuaweiCloudServerGroupView;
}

export class HuaweiCloudServerGroupActions extends React.Component<IHuaweiCloudServerGroupActionsProps> {
  ...
  private rollbackServerGroup = (): void => {
    ...
    let serverGroup: IHuaweiCloudServerGroup = this.props.serverGroup;
    let previousServerGroup: IHuaweiCloudServerGroup;
    ...
    if (serverGroup.isDisabled) {
       ...
       */
      serverGroup = orderBy(
        allServerGroups.filter((g: IHuaweiCloudServerGroup) => g.name !== previousServerGroup.name && !g.isDisabled),
        ['instanceCounts.total', 'createdTime'],
        ['desc', 'desc'],
      )[0] as IHuaweiCloudServerGroup;
    }

    // the set of all server groups should not include the server group selected for rollback
    allServerGroups = allServerGroups.filter((g: IHuaweiCloudServerGroup) => g.name !== serverGroup.name);
  ...
  private resizeServerGroup = (): void => {
    const { app, serverGroup } = this.props;
    HuaweiCloudResizeServerGroupModal.show({ application: app, serverGroup });
  };

  private cloneServerGroup = (): void => {
    const { app, serverGroup } = this.props;
    HuaweiReactInjector.huaweicloudServerGroupCommandBuilder
      .buildServerGroupCommandFromExisting(app, serverGroup)
      .then((command: IHuaweiCloudServerGroupCommand) => {
        const title = `Clone ${serverGroup.name}`;
        HuaweiCloudCloneServerGroupModal.show({ title, application: app, command });
      });
  };
