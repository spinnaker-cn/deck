...
export class SecurityGroups extends React.Component<ISecurityGroupsProps, ISecurityGroupsState>
  ...
  constructor(props: ISecurityGroupsProps) {
    ...
    const defaultSecurityGroups = get<string[]>(HuaweiProviderSettings, 'defaultSecurityGroups', []);
    this.state = {
  ...
  public validate(values): FormikErrors<IHuaweiCloudLoadBalancerUpsertCommand> {
    ...
    if (!values.securityGroups || !values.securityGroups.length) {
      return { Firewalls: 'Firewall required' }
    }
    return {};
  }
  ...
  private updateRemovedSecurityGroups(selectedGroups: string[], availableGroups: ISecurityGroup[]): void {
    ...
    const getAvailableSecurityGroup = (id: string) => availableGroups.find(sg => sg.name === id || sg.id === id);

    // Organize selected security groups into available/not available
    const [available, notAvailable] = partition(getDesiredGroupNames(), id => !!getAvailableSecurityGroup(id));

    // Normalize available security groups from [name or id] to name
    const securityGroups = available.map(id => getAvailableSecurityGroup(id).id);
  ...
  public componentDidMount(): void {
    ...
    const availableSecurityGroups$ = Observable.combineLatest(vpcId$, allSecurityGroups$)
      ...
      .map(([[vpcId, allSecurityGroups], formValues]) => {
        ...
        const forRegion = (forAccount.huaweicloud && forAccount.huaweicloud[formValues.region]) || [];
    ...
    availableSecurityGroups$
      ...
      .subscribe(([availableSecurityGroups, formValues]) => {
        const makeOption = (sg: ISecurityGroup) => ({ label: `${sg.name} (${sg.id})`, value: sg.id });
