...
import { IHuaweiCloudLoadBalancer, IHuaweiCloudLoadBalancerUpsertCommand } from 'huaweicloud/domain';
import { SubnetSelectField } from 'huaweicloud/subnet';
...
export interface ILoadBalancerLocationProps {
  app: Application;
  formik: FormikProps<IHuaweiCloudLoadBalancerUpsertCommand>;
  forPipelineConfig?: boolean;
  isNew?: boolean;
  loadBalancer?: IHuaweiCloudLoadBalancer;
}
...
export class LoadBalancerLocation extends React.Component<ILoadBalancerLocationProps, ILoadBalancerLocationState>
  implements IWizardPageComponent<IHuaweiCloudLoadBalancerUpsertCommand> {
  ...
  public validate(values: IHuaweiCloudLoadBalancerUpsertCommand) {
    const errors = {} as FormikErrors<IHuaweiCloudLoadBalancerUpsertCommand>;
  ...
  private shouldHideInternalFlag(): boolean {
    if (HuaweiProviderSettings) {
      if (HuaweiProviderSettings.loadBalancers && HuaweiProviderSettings.loadBalancers.inferInternalFlagFromSubnet) {
  ...
  public componentDidMount(): void {
    ...
    const allAccounts$ = Observable.fromPromise(AccountService.listAccounts('huaweicloud')).shareReplay(1);
    ...
    const allLoadBalancers$ = this.props.app.getDataSource('loadBalancers').data$ as Observable<IHuaweiCloudLoadBalancer[]>;
    ...
    subnet$.takeUntil(this.destroy$).subscribe(subnet => {
      ...
      this.props.formik.setFieldValue('subnetId', subnet && subnet.purpose);
    });
  ...
  private getAvailableSubnets(credentials: string, region: string): IPromise<ISubnet[]> {
    return SubnetReader.listSubnetsByProvider('huaweicloud').then(subnets => {
  ...
  private handleSubnetUpdated = (subnetId: string): void => {
    this.props.formik.setFieldValue('subnetId', subnetId);
  };
          ...
          <div className="modal-body">
            <div className="form-group">
              <div className={className}>
                ...
                <HelpField id="huaweicloud.loadBalancer.name" />
            ...
            <div className="form-group">
              <div className="col-md-3 sm-label-right">
                Stack <HelpField id="huaweicloud.loadBalancer.stack" />
              </div>
              ...
              <div className="col-md-6 form-inline">
                <label className="sm-label-right">
                  <span>
                    Detail <HelpField id="huaweicloud.loadBalancer.detail" />{' '}
                  </span>
                </label>
                <input
                  type="text"
            ...
            <SubnetSelectField
              labelColumns={3}
              helpKey="huaweicloud.loadBalancer.subnet"
              component={values}
              field="subnetId"
              region={values.region}
              subnets={subnets as any}
              application={app}
              onChange={() => this.handleSubnetUpdated(values.subnetId)}
            />
          </div>
        )}
      </div>
