...
import {
  ...
  IHuaweiCloudCertificate,
  IHuaweiCloudClassicLoadBalancerUpsertCommand,
  IClassicListenerDescription,
} from 'huaweicloud/domain';
import { HuaweiCloudCertificateReader } from 'huaweicloud/certificates/HuaweiCloudCertificateReader';
import { HuaweiCloudCertificateSelectField } from '../common/HuaweiCloudCertificateSelectField';

import './Listeners.less';

export interface IListenersProps {
  formik: FormikProps<IHuaweiCloudClassicLoadBalancerUpsertCommand>;
  app: Application;
}

export interface IListenersState {
  certificates: { [accountId: number]: IHuaweiCloudCertificate[] };
}
...
export class Listeners extends React.Component<IListenersProps, IListenersState> {
  ...
  private loadCertificates(): void {
    HuaweiCloudCertificateReader.listCertificates().then(certificates => {
  ...
  private renderCertificateSelector(listener: IClassicListenerDescription): JSX.Element {
    if (!this.secureProtocols.includes(listener.externalProtocol)) {
      ...
      return (
        <HuaweiCloudCertificateSelectField
