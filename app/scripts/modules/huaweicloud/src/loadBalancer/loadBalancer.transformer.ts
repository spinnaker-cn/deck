...
import {
  ...
  IHuaweiCloudApplicationLoadBalancer,
  IHuaweiCloudApplicationLoadBalancerUpsertCommand,
  IHuaweiCloudClassicLoadBalancer,
  IHuaweiCloudClassicLoadBalancerUpsertCommand,
  IHuaweiCloudLoadBalancer,
  IHuaweiCloudServerGroup,
  ...
  IHuaweiCloudNetworkLoadBalancerUpsertCommand,
  ITargetGroup,
} from 'huaweicloud/domain';
import { VpcReader } from 'huaweicloud/vpc/VpcReader';
import { IPromise, module } from 'angular';
import { chain, filter, flatten, map } from 'lodash';

import { $q } from 'ngimport';

export class HuaweiLoadBalancerTransformer {
  private updateHealthCounts(container: IServerGroup | ITargetGroup | IHuaweiCloudLoadBalancer): void {
    ...
    if ((container as ITargetGroup | IHuaweiCloudLoadBalancer).serverGroups) {
  ...
  private addVpcNameToContainer(
    container: IHuaweiCloudLoadBalancer | ITargetGroup,
  ): (vpcs: IVpc[]) => IHuaweiCloudLoadBalancer | ITargetGroup {
    return (vpcs: IVpc[]) => {
  ...
  public normalizeLoadBalancer(loadBalancer: IHuaweiCloudLoadBalancer): IPromise<IHuaweiCloudLoadBalancer> {
    ...
    if ((loadBalancer as IHuaweiCloudApplicationLoadBalancer).targetGroups) {
      const appLoadBalancer = loadBalancer as IHuaweiCloudApplicationLoadBalancer;
      appLoadBalancer.targetGroups.forEach(targetGroup => this.normalizeTargetGroup(targetGroup));
      serverGroups = flatten<IHuaweiCloudServerGroup>(map(appLoadBalancer.targetGroups, 'serverGroups'));
    }
    ...
    return VpcReader.listVpcs().then(
      (vpcs: IVpc[]) => this.addVpcNameToContainer(loadBalancer)(vpcs) as IHuaweiCloudLoadBalancer,
    );
  }
  ...
  public convertClassicLoadBalancerForEditing(
    ...
    const toEdit: IHuaweiCloudClassicLoadBalancerUpsertCommand = {
  ...
  public convertApplicationLoadBalancerForEditing(
    ...
    const toEdit: IHuaweiCloudApplicationLoadBalancerUpsertCommand = {
      availabilityZones: undefined,
  ...
  public convertNetworkLoadBalancerForEditing(
    ...
    // Since we build up toEdit as we go, much easier to declare as any, then cast at return time.
    const toEdit: IHuaweiCloudNetworkLoadBalancerUpsertCommand = {
      availabilityZones: undefined,
      isInternal: loadBalancer.isInternal,
      region: loadBalancer.region,
  ...
  public constructNewClassicLoadBalancerTemplate(application: Application): IHuaweiCloudClassicLoadBalancerUpsertCommand {
    const defaultCredentials = application.defaultCredentials.huaweicloud || HuaweiProviderSettings.defaults.account,
      defaultRegion = application.defaultRegions.huaweicloud || HuaweiProviderSettings.defaults.region,
      defaultSubnetType = HuaweiProviderSettings.defaults.subnetType;
    return {
  ...
  public constructNewNetworkLoadBalancerTemplate(application: Application): IHuaweiCloudNetworkLoadBalancerUpsertCommand {
    const defaultCredentials = application.defaultCredentials.huaweicloud || HuaweiProviderSettings.defaults.account,
      defaultRegion = application.defaultRegions.huaweicloud || HuaweiProviderSettings.defaults.region,
      defaultSubnetType = HuaweiProviderSettings.defaults.subnetType,
      defaultTargetGroupName = `targetgroup`;
...
export const AWS_LOAD_BALANCER_TRANSFORMER = 'spinnaker.huaweicloud.loadBalancer.transformer';
module(AWS_LOAD_BALANCER_TRANSFORMER, []).service('huaweicloudLoadBalancerTransformer', HuaweiLoadBalancerTransformer);
