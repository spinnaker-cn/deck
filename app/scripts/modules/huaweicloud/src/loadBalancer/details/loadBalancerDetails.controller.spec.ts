...
import { AWS_LOAD_BALANCER_DETAILS_CTRL, HuaweiLoadBalancerDetailsController } from './loadBalancerDetails.controller';

describe('Controller: LoadBalancerDetailsCtrl', function() {
  let controller: HuaweiLoadBalancerDetailsController;
  let $scope;
  let $state;
  const loadBalancer = {
    name: 'foo',
  ...
  beforeEach(
    mock.inject(($controller: IControllerService, $rootScope: IRootScopeService, _$state_: StateService) => {
      ...
      controller = $controller(HuaweiLoadBalancerDetailsController, {
        $scope,
        loadBalancer,
        app,
        $state,
