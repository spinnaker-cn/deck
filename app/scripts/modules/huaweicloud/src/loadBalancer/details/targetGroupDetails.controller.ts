...
import { IHuaweiCloudApplicationLoadBalancer, ITargetGroup } from 'huaweicloud/domain/IHuaweiCloudLoadBalancer';
...
export class HuaweiTargetGroupDetailsController implements IController {
  ...
  public loadBalancer: IHuaweiCloudApplicationLoadBalancer;
  ...
  public extractTargetGroup(): IPromise<void> {
    ...
    const appLoadBalancer: IHuaweiCloudApplicationLoadBalancer = this.app.loadBalancers.data.find((test: ILoadBalancer) => {
...
export const AWS_TARGET_GROUP_DETAILS_CTRL = 'spinnaker.huaweicloud.loadBalancer.details.targetGroupDetails.controller';
module(AWS_TARGET_GROUP_DETAILS_CTRL, [require('@uirouter/angularjs').default]).controller(
  'huaweicloudTargetGroupDetailsCtrl',
  HuaweiTargetGroupDetailsController,
);
