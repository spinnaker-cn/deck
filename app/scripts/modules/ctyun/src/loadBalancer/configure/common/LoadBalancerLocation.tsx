...
import { eipReader } from 'ctyun/eip/eipReader';
...
import { IAmazonLoadBalancer, IAmazonLoadBalancerUpsertCommand } from 'ctyun/domain';
...
export class LoadBalancerLocation extends React.Component<ILoadBalancerLocationProps, ILoadBalancerLocationState>
  ...
  public validate(values: IAmazonLoadBalancerUpsertCommand) {
    ...
    if (!values.isInternal && !values.eipId) {
      errors.eipId = 'Please select elastic IP';
    }
  ...
  public componentDidMount(): void {
    ...
    const allAccounts$ = Observable.fromPromise(AccountService.listAccounts('ctyun')).shareReplay(1);
    ...
    eipReader.listEips(this.props.formik.values.credentials, this.props.formik.values.region).then((eips: any[]) => {
      this.props.formik.setFieldValue('eipsList', eips);
      if (eips.length > 0) {
        this.props.formik.setFieldValue('eipId', eips[0].value);
      }
    });
  }
  ...
  private getAvailableSubnets(credentials: string, region: string): IPromise<ISubnet[]> {
    return SubnetReader.listSubnetsByProvider('ctyun').then(subnets => {
  ...
  private eipIDChanged = (event: React.ChangeEvent<any>): void => {
    this.props.formik.setFieldValue('eipId', event.target.value);
  };
  ...
  private regionUpdated = (region: string): void => {
    ...
    eipReader.listEips(this.props.formik.values.credentials, region).then((eips: any[]) => {
      this.props.formik.setFieldValue('eipsList', eips);
      if (eips.length > 0) {
        // if (!this.props.formik.values.eipId) {
        this.props.formik.setFieldValue('eipId', eips[0].value);
        // }
      } else {
        this.props.formik.setFieldValue('eipId', '');
      }
    });
  };
      ...
      <div className="container-fluid form-horizontal">
        ...
        {accounts && (
          <div className="modal-body">
            ...
            {!values.isInternal && (
              <div className="form-group">
                <div className="col-md-3 sm-label-right">Elastic IP</div>
                <div className="col-md-7">
                  <select
                    className="form-control input-sm"
                    value={values.eipId}
                    onChange={this.eipIDChanged}
                    required={true}
                  >
                    <option value="" disabled={true}>
                      Select...
                    </option>
                    {values.eipsList.map(eips => {
                      return (
                        <option key={eips.value} value={eips.label}>
                          {eips.eipAddress}
                        </option>
                      );
                    })}
            ...
            {errors.eipId && (
              <div className="col-md-7 col-md-offset-3">
                <ValidationMessage type="error" message={errors.eipId} />
              </div>
            )}
          </div>
        )}
