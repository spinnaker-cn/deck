...
import { IAuthenticateOidcActionConfig } from 'ctyun/loadBalancer/OidcConfigReader';
...
export class ALBListeners extends React.Component<IALBListenersProps, IALBListenersState>
  ...
  private targetGroupNameChanged(listener: IListenerDescription, newName: string): void {
    listener.targetGroupName = newName;
    this.updateListeners();
  }
                ...
                <div>
                  <div className="wizard-pod-row header">
                    ...
                    <div className="wizard-pod-row-contents spread">
                      <div className="listenerFlexContent">
                        ...
                        <span className="wizard-pod-content mgTop">
                          <label>Service Group Name</label>
                          <input
                            disabled={!listener.isNew}
                            className="form-control input-sm inline-number"
                            type="text"
                            value={listener.targetGroupName || ''}
                            onChange={event => this.targetGroupNameChanged(listener, event.target.value)}
                            style={{ width: '150px' }}
                            required={true}
                          />
                        </span>
...
interface IHealthCheckProps {
  ...
  isL7: boolean;
  isNewListener: boolean;
  handleHealthCheckChanged: (healthCheck: ICtyunHealthCheck, key: string, newValue: string | number | boolean) => void;
                  ...
                  <VirtualizedSelect
                    ...
                    onChange={(options: IOption[]) => {
                      handleHealthCheckChanged(
                        healthCheck,
                        'httpCode',
                        options.map(({ value }) => value as String).toString(),
                      );
                    }}
