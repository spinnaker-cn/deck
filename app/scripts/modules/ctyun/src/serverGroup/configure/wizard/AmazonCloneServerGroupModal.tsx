...
> {
  ...
  private initializeCommand = () => {
    ...
    AwsReactInjector.ctyunServerGroupConfigurationService.configureSubnetPurposes(command);
  };

  private configureCommand = () => {
    const { application, command } = this.props;
    AwsReactInjector.ctyunServerGroupConfigurationService.configureCommand(application, command).then(() => {
      if (command.backingData.filtered.lbList) {
        AwsReactInjector.ctyunServerGroupConfigurationService.refreshLoadBalancerListenerMap(command).then(() => {
      ...
      <WizardModal<IAmazonServerGroupCommand>
        ...
        render={({ formik, nextIdx, wizard }) => (
          <>
            ...
            {command.optionZone && command.optionZone.length > 0 && (
              <WizardPage
                label="Availability Zone"
                wizard={wizard}
                order={nextIdx()}
                render={({ innerRef }) => (
                  <ServerGroupAvailabilityZone ref={innerRef} formik={formik} app={application} />
                )}
              />
            )}
