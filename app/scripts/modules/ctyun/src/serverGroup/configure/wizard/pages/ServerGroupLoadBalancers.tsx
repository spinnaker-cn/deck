...
interface ICtyunLocationMap {
  [key: string]: ICtyunLocation;
}
export interface IServerGroupLoadBalancersState {
  refreshing: boolean;
  listenerLocationMap: ICtyunLocationMap;
}
...
export class ServerGroupLoadBalancers
  ...
  public state = {
    refreshing: false,
    listenerLocationMap: {} as ICtyunLocationMap,
  };
  ...
  private loadBalancerChanged(forwardLoadBalancer: ICtyunForwardLoadBalancer, loadBalancerId: string): void {
  ...
  private listenerChanged(forwardLoadBalancer: ICtyunForwardLoadBalancer, listenerId: string, index: number) {
    ...
    forwardLoadBalancer.hostGroupID = selectedListener.targetGroupId;
    this.setState({
      listenerLocationMap: {
  ...
  private portChanged(forwardLoadBalancer: ICtyunForwardLoadBalancer, port: number) {
    forwardLoadBalancer.targetAttributes[0].port = port;
    this.updateLoadBalancers();
  }

  private weightChanged(forwardLoadBalancer: ICtyunForwardLoadBalancer, weight: number) {
  ...
  private domainChanged = (forwardLoadBalancer: ICtyunForwardLoadBalancer, domain: string, index: number) => {
    const { listenerLocationMap } = this.state;
    forwardLoadBalancer.locationId = '';
  ...
  private urlChanged = (forwardLoadBalancer: ICtyunForwardLoadBalancer, url: string, index: number) => {
    const { listenerLocationMap } = this.state;
    const listenerLocation = listenerLocationMap[index];
    ...
    forwardLoadBalancer.hostGroupID = rule.ruleTargetGroupId;
    this.updateLoadBalancers();
    this.setState({
      listenerLocationMap: {
        ...listenerLocationMap,
  ...
  public componentWillReceiveProps(nextProps: IServerGroupLoadBalancersProps): void {
    const {
      values: {
        ...
        backingData: {
          filtered: { lbListenerMap = {} as ICtyunLbListenerMap },
        },
    ...
    if (
      ...
      this.setState({
        listenerLocationMap: Object.assign(
          {},
          this.state.listenerLocationMap,
          forwardLoadBalancers.reduce((p: ICtyunLocationMap, c, index) => {
