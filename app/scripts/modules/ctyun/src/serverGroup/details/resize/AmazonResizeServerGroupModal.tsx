...
import { IServerGroup, ITask } from '@spinnaker/core';

import { AwsModalFooter } from 'ctyun/common';
import { IAmazonAsg, IAmazonServerGroup } from 'ctyun/domain';
...
> {
  ...
  constructor(props: IAmazonResizeServerGroupModalProps) {
    ...
    this.state = {
      ...
      interestingHealthProviderNames:
        attributes.platformHealthOnlyShowOverride && attributes.platformHealthOnly ? ['ctyun'] : null,
    };
  }
  ...
  private validate = (
    ...
    if (min > max && min > desired) {
      ...
      errors.max = 'Max cannot be smaller than Min';
    } else {
      if (min > max) {
        errors.min = 'Min cannot be larger than Max';
      }
  ...
  private submit = (values: IAmazonResizeServerGroupValues): void => {
    ...
    if (!capacity.min && capacity.min != 0) {
      capacity.min = asg.minCount;
    }
    if (!capacity.max && capacity.max != 0) {
      capacity.max = asg.maxCount;
    }
    if (!capacity.desired && capacity.desired != 0) {
      capacity.desired = asg.instanceCount;
    }
    ...
    this.state.taskMonitor.submit(() => {
      return this.resizeServerGroup(serverGroup, application, command);
    });
  };
  ...
  private resizeServerGroup(
    ...
    const currentSize: string = this.getCapacityString({
        ...
        <div className="form-group">
          ...
          <div className="col-md-4">
            <div className="horizontal middle">
              <FormikFormField
                name="desired"
                input={props => <NumberInput {...props} min={0} max={50} />}
                touched={true}
                onChange={value => {
      ...
      <div>
        ...
        <div className="form-group bold">
          <div className="col-md-2 col-md-offset-3">
            Min <HelpField id="aws.serverGroup.Min" />
          </div>
          <div className="col-md-2">
            Max <HelpField id="aws.serverGroup.Max" />
          </div>
