...
import { IScalingPolicyAlarm, ITargetTrackingConfiguration } from 'ctyun/domain';
...
const component: IComponentOptions = {
  ...
  template: `
    <ctyun-metric-alarm-chart alarm="$ctrl.alarm"
                        style="height: 150px;"
                        alarm-updated="$ctrl.alarmUpdated"
                        ticks="{x: 12, y: 5}"
                        margins="{top: 10, left: 50}"
                        stats="$ctrl.state"
                        server-group="$ctrl.serverGroup"></ctyun-metric-alarm-chart>
  `,
};

export const TARGET_TRACKING_CHART_COMPONENT = 'spinnaker.ctyun.scalingPolicy.targetTracking.chart.component';
