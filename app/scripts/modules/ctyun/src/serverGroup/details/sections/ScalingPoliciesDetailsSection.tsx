...
import { IAmazonServerGroupView, IScalingProcess } from 'ctyun/domain';
...
export class ScalingPoliciesDetailsSection extends React.Component<IAmazonServerGroupDetailsSectionProps> {
  ...
  public render(): JSX.Element {
    ...
    serverGroup.scalingPolicies = serverGroup.scalingPolicies.filter(p => p.ruleType == 1);
    return (
      <CollapsibleSection
      ...
      >
        ...
        {serverGroup.scalingPolicies.map(policy => (
          <ScalingPolicySummary key={policy.ruleID} policy={policy} serverGroup={serverGroup} application={app} />
        ))}
