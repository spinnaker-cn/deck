      ...
      return (
        <CollapsibleSection heading="Launch Configuration">
          <dl className="horizontal-when-filters-collapsed">
            <dt>Name</dt>
            <dd>{launchConfig.name}</dd>

            <dt>Image ID</dt>
            <dd>{launchConfig.imageID}</dd>

            {launchConfig && launchConfig.imageName && <dt>Image Name</dt>}
            {launchConfig && launchConfig.imageName && <dd>{launchConfig.imageName}</dd>}

            {image && image.baseImage && <dt>Base Image Name</dt>}
            {image && image.baseImage && <dd>{image.baseImage}</dd>}

            <dt>Instance Type</dt>
            <dd>{launchConfig.specName}</dd>

            {/* <dt>Security Service</dt>
            <dd>
              {launchConfig.enhancedService && launchConfig.enhancedService.securityService.enabled
                ? 'enabled'
                : 'disabled'}
            </dd> */}
            ...
            <dd>
              {true || (launchConfig.enhancedService && launchConfig.enhancedService.monitorService.enabled)
                ? 'enabled'
                : 'disabled'}
            </dd>
