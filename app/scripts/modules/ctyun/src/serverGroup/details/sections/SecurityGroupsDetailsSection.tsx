...
> {
  ...
  private getSecurityGroups(props: IAmazonServerGroupDetailsSectionProps): ISecurityGroup[] {
    ...
    if (props.serverGroup.asg && serverGroup.asg.securityGroupIDList) {
      securityGroups = chain(serverGroup.asg.securityGroupIDList)
        .map((id: string) => {
          return (
  ...
  public render(): JSX.Element {
    ...
    return (
      <CollapsibleSection heading={FirewallLabels.get('Firewalls')}>
        ...
        {securityGroups.length == 0 && <span>none</span>}
      </CollapsibleSection>
    );
  }
}
