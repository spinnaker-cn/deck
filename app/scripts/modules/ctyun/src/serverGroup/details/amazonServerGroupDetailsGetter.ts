...
import { IAmazonLoadBalancer, IAmazonServerGroup, IAmazonServerGroupView } from 'ctyun/domain';
  ...
  return new Observable<IAmazonServerGroupView>(observer => {
    extractServerGroupSummary(props).then(summary => {
      ServerGroupReader.getServerGroup(
        ...
        const serverGroup = AwsReactInjector.ctyunServerGroupTransformer.normalizeServerGroupDetails(details);
