...
module.exports = angular
  .module('spinnaker.ctyun.securityGroup.baseConfig.controller', [
  ...
  .controller('ctyunConfigSecurityGroupMixin', [
    ...
    function($scope, $state, $uibModalInstance, application, securityGroup, securityGroupReader) {
      ...
      ctrl.initializeAccounts = () => {
        return AccountService.listAllAccounts('ctyun').then(function(accounts) {
      ...
      ctrl.regionUpdated = function() {
        ...
        let item = $scope.allVpcs.find(da => da.value == $scope.securityGroup.vpcId);
        if (!item || item.region != $scope.securityGroup.region) {
          $scope.securityGroup.vpcId = '';
        }
        $scope.vpclist = $scope.allVpcs.filter(item => item.region == $scope.securityGroup.region);
      };
      ...
      function configureFilteredSecurityGroups() {
        ...
        var regionalGroupNames = _.get(allSecurityGroups, [account, 'ctyun', region].join('.'), []).map(sg => sg.name);
      ...
      ctrl.initializeSecurityGroups = function() {
        return securityGroupReader.getAllSecurityGroups().then(function(securityGroups) {
          ...
          normalizeSecurityGroup(securityGroups).then(data => {
            $scope.allVpcs = data.map(item => ({
          ...
          var availableGroups;
          if (account && region) {
            availableGroups = (securityGroups[account] && securityGroups[account].ctyun[region]) || [];
          } else {
