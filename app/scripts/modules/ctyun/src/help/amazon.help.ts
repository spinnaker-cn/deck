...
const helpContents: { [key: string]: string } = {
  ...
  'aws.serverGroup.imageName':
    '(Required) <b>Image</b> is the deployable ctyun Machine Image. Images are restricted to the account and region selected.',
  ...
  'aws.serverGroup.securityService':
  ...
  'aws.serverGroup.Min': `The minimum number of instances ranges from 0 to 50.`,
  'aws.serverGroup.Max': `The maximum number of instances should be a number greater than 1.`,
  'aws.serverGroup.releaseMode': `Refers to the processing strategy for cloud hosts after removal.
    <p>1. Release mode: Refers to the release of the removed pay as you go cloud host (including public IP);
    For cloud hosts with annual and monthly packages, if they are moved out of the scaling group, they will not be deleted or their status will be changed.</p>`,
  'aws.serverGroup.healthMode': `The health check will remove the abnormal cloud server from the scaling group and recreate a new cloud server. Including cloud server health check (default) and elastic load balancing health check.
    <p>A. <b>Cloud server health check</b>: refers to checking the running status of the cloud server, such as shutting down or deleting, which are abnormal states of the cloud server.</p>
    <p>B. <b>Elastic load balancing health check</b>: refers to the check conducted on the health check results of cloud servers based on load balancing. The cloud server status detected under all listeners must be normal.</p>`,
  'aws.serverGroup.moveOutStrategy': `<p>Older configurations created earlier: First filter out instances created by earlier configurations, and then filter out instances created earlier.</p>
    <p>Earlier created instances: Filter instances created earlier based on time, regardless of whether they were created in a scaling configuration.</p>`,
};
