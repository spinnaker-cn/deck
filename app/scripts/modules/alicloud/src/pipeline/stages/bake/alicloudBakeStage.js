...
module.exports = angular
  .module('spinnaker.alicloud.pipeline.stage.bakeStage', [require('./bakeExecutionDetails.controller').name])
  .config(function() {
  ...
  .controller('alicloudBakeStageCtrl', [
    '$scope',
    '$q',
    '$uibModal',
    function($scope, $q, $uibModal) {
      ...
      function initialize() {
        $q.all({
          regions: BakeryReader.getRegions('alicloud'),
          baseOsOptions: BakeryReader.getBaseOsOptions('alicloud'),
          ...
          if (!$scope.stage.regions.length && $scope.application.defaultRegions.alicloud) {
            $scope.stage.regions.push($scope.application.defaultRegions.alicloud);
          }
          if (!$scope.stage.regions.length && $scope.application.defaultRegions.alicloud) {
            $scope.stage.regions.push($scope.application.defaultRegions.alicloud);
          }
          if (results.baseOsOptions) {
